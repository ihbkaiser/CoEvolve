...
class AnalysisReflection:
    ...
    def generate_prompt_for_plan_reflection(self, iteration: int, error_analysis: Dict, problem: str, problem_understanding: str, plan: str, historical_logs: Dict) -> str:
        ...
        prompt = f"""
You are a **debugging assistant** for a competitive programming problem.
# Context Provided:
## Problem:
{problem}
...
## New Insights:
{insights}
## Previous Reflection Prompt From Previous Iteration:
{previous_reflection}
## Success Rate from Previous Iteration:
{success_rate_str}
---
Your response must be structured as follow:
### Error Localization
Identify errors or omissions in the plan that cause incorrect input/output
### Plan Improvement Suggestions
Provide specific suggestions for improving the plan for the next iteration based on the error localization. Plan may be slightly modified, or have to switch to a different approach
---
⚠️ **IMPORTANT:**
- Your task is to look at the insight and reflection prompt from the previous iteration, and bring a reflection prompt to the current iteration. Not bring the exact same reflection prompt.
- **Do not generate code.**
- **The test cases are always correct — never question their validity.**
"""
        return prompt
class CoEvolve_final(BaseStrategy):
    def __init__(
        self,
        k: int = 1,
        t: int = 4,
    ...
    def compute_score(self, test_log: str) -> float:
        ...
        num_passed = test_log.count("passed")
        num_failed = test_log.count("failed")
        ...
        return score
    def get_failed_test_cases(self, test_log: str) -> str:
        failed_cases = []
...
"""
    ...
    def plan_analysis(self, plan: str, test_log: str, problem: str, problem_understanding: str, code=None) -> PlanAnalysisOutput:
        if self.verbose:
            print("Step: Performing plan analysis")
            print(f"Plan: {plan}...")
        code_prompt = "" if code is None else f"### Code\n{code}\n"
        input_prompt = [
            {
                "role": "user",
                "content":
                f"""
**You are a code generation assistant tasked with analyzing a programming problem in debugging and logical reasoning.**
---
## Context:
### Problem Description
{problem}
...
{self.get_failed_test_cases(test_log)}
---
## Primary Task
Perform a **detailed analysis** of the plan, including **error localization**. Specifically:
- **Evaluate feasibility** of the current plan
- Decide if the plan needs to be **updated** or if only the **code needs adjustment**
- If an update is required, explain clearly **how to update the plan**
---
## Error Localization Task
For each step of the plan:
1. **Trace the logic** using the failing test case input
2. Identify where the simulation results **diverge** from the expected output in the test log
3. Localize the specific **logical flaws**, **incorrect assumptions**, or **mismatches** in the plan
---
## Response Structure
...
### Insight
Provide insights on the plan:
- Does the plan itself need updating due to a flawed approach? If yes, explain how to update it based on the simulation and problem understanding.
- Or is the plan correct, but the code is wrong? If so, confirm the correctness of the plan.
---
## IMPORTANT
- **Strictly follow** the instructions.
- The **test log is always true**. Do not modify or doubt it.
- Do not be overconfident. The **current plan or code has issues**.
- **Only focus** on plan analysis. Do **not** generate new code.
"""
            },
        ]
        pr_tok = 0
        ...
        for attempt in range(self.max_attempts):
            ...
            try:
                ...
                print(f"Response from plan analysis: {response}")
                pr_tok += pr_tok_temp
                com_tok += com_tok_temp
                parsed = PlanAnalysisOutput(
                    simulation=self.parse_key_from_md(response, "Simulation"),
                    insights=self.parse_key_from_md(response, "Insight"),
                    pr_tok=pr_tok,
                    com_tok=com_tok
                )
                if self.verbose:
...
### Problem Understanding
{problem_understanding}
### Code Implementation ({self.language})
```{self.language}
{code}
```
### Test Log (failing input/output)
...
---
## Primary Task
Perform a **detailed analysis** of the code, including **error localization**. Specifically:
* **Evaluate the issues** in the code
* Provide **actionable insights** for refinement
---
## Error Localization Task
Execute the code **line-by-line** to:
1. Pinpoint and localize specific **errors**, **mismatches**, or **logical flaws**
2. Trace where the behavior diverges from the expected results in the test log
---
## Response Structure
Your response must be structured as follow:
### Simulation
...
### Insight
Provide insights on the code:
* Summarize the identified flaws based on the simulation and problem understanding
* Offer **actionable suggestions** for refinement
---
## IMPORTANT
* **Strictly follow** the instructions.
* The **test log is always true**. Do not modify or doubt it.
* The **current code is wrong**.
* **Only focus** on code analysis. Do **not** generate new code, just focus on insights into how the code should be modified.
"""
    },
]
        pr_tok = 0
        ...
        for attempt in range(self.max_attempts):
            ...
            try:
                response, pr_tok_temp, com_tok_temp = self.gpt_chat(input_prompt)
                print(f"Response from code analysis: {response}")
                pr_tok += pr_tok_temp
                com_tok += com_tok_temp
                parsed = CodeAnalysisOutput(
                    simulation=self.parse_key_from_md(response, "Simulation"),
                    insights=self.parse_key_from_md(response, "Insight"),
                    pr_tok=pr_tok,
                    com_tok=com_tok
        ...
        input_prompt = [
    {
        ...
        "content":
f"""
**You are a code generation assistant tasked with analyzing a programming problem in assessing solution alignments.**
---
## Context:
### Problem Description
{problem}
...
---
## Primary Task
Analyze how well the **plan** and **code** align with the **problem requirements** and with each other
Identify any mismatches, omissions, unnecessary steps, logical inconsistencies or implementation deviations.
---
## Response Requirement
Provide a **single concise insight** that includes:
...
## IMPORTANT
* **Strictly follow** the instructions.
* **Focus only** on alignment issues with a concise insight.
...
"""
        ...
        for attempt in range(self.max_attempts):
            ...
            try:
                ...
                pr_tok += pr_tok_temp
                com_tok += com_tok_temp
                parsed = ContentAnalysisOutput(
                    plan_code_insights=response.strip(),
                    pr_tok=pr_tok,
                    com_tok=com_tok
                )
                if self.verbose:
    ...
    def get_all_confidence(self, decisions: List[str], analyses: Dict[str, Dict]) -> Dict[str, Dict[str, ConfidenceOutput]]:
        """
        Tráº£ vá»: { decision: { analysis_type: ConfidenceOutput } }
        """
        if self.verbose:
            print("Step: Computing all confidence scores in a single API call")
        # Cá»‘ Ä‘á»‹nh thá»© tá»± & chá»‰ giá»¯ 3 loáº¡i há»£p lá»‡
        ...
        user_content = (
            ...
            "**Scoring rules** (confidence in [0.0, 1.0]):\n"
        ...
        for attempt in range(self.max_attempts):
            ...
            try:
                # Náº¿u cÃ³, nÃªn báº­t response_format='json' Ä‘á»ƒ khÃ³a Ä‘áº§u ra JSON
    ...
    def get_all_consistency(self, decisions: List[str], analyses: Dict[str, Dict]) -> Dict[str, Dict[str, ConsistencyOutput]]:
        """
        Tráº£ vá»: { decision: { "name1-name2": ConsistencyOutput } }
        """
        if self.verbose:
        ...
        user_content = (
            ...
            "**Scoring rules** (consistency in [0.0, 1.0]):\n"
    ...
    def perform_analyses(self, plan: str, code: str, test_log: str, problem: str, problem_understanding: str) -> Dict:
        ...
        task_dictionary = {
            'plan': lambda: self.plan_analysis(plan, test_log, problem, problem_understanding, code),
                    ...
                    if decision == 'update plan':
                        ...
                        try:
                            ...
                            try:
                                ...
                                new_score = self.compute_score(new_test_log)
                            except Exception as e:
